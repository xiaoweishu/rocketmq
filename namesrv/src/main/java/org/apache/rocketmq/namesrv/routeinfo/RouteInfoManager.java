...
public class RouteInfoManager {
    ...
    /**
     * Topic 消息队列路由信息，消息发送时根据路由表进行负 载均衡<br/>
     * 一个Broker为每一主题默认创建 4 个读队列 4 个写队列
     */
    private final HashMap<String/* topic */, List<QueueData>> topicQueueTable;
    /**
     * Broker 基础信息， 包含 brokerName、 所属集群名称 、 主备 Broker地址<br/>
     * 多个 Broker 组成 一个集群 ， BrokerName 由相同的多台 Broker 组 成 Master-Slave 架构 ， brokerId 为 0 代表 Master， 大于 0 表示 Slave
     */
    private final HashMap<String/* brokerName */, BrokerData> brokerAddrTable;
    /**
     * Broker 集群信息，存储集群中所有 Broker 名称<br/>
     */
    ...
    public RegisterBrokerResult registerBroker(
        ...
        try {
            try {
                // 多线程，锁，细节：路由注册需要加写锁 ，防止并发修改RouteInfoManager中的路由表,允许多个消息发送者( Producer)并发读， 保证消息发送时的高并发
                // 同一时刻 NameServer 只处理一个 Broker 心跳包，多个心跳包请求串行执行
                this.lock.writeLock().lockInterruptibly();
                // Step1:维护集群信息
                ...
                // Step4: 更新 BrokerLiveInfo，BrokeLiveInfo是执行路由删除的重要依据
